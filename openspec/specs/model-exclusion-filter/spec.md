...
- **WHEN** a model has a slug that starts with a configured excluded prefix, coding score, and is not deprecated
- **THEN** the system SHALL exclude that model from the ranking

#### Scenario: Model with non-excluded slug prefix is ranked

- **WHEN** a model has a slug that does not start with any configured excluded prefix, coding score, and is not deprecated
...
- **THEN** the system SHALL include all eligible models in the ranking
